...
| R11 | Information Disclosure | `/docs` (Swagger UI) y `/openapi.json` accesibles sin autenticación, exponiendo el esquema completo de la API (los 4 endpoints, parámetros y modelos de datos) | `curl http://100.110.229.99/docs` y `/openapi.json`; confirmado en tráfico capturado (Wireshark, `tcp.stream eq 1` y `eq 2`, ver `evidence-lab3-http-plaintext.txt`); ZAP pasivo (`reports/zap-passive/zap_report.html`) | Alta | Medio | Alta | 🔵 Pendiente (Fase E) | Restringir o deshabilitar `/docs` y `/openapi.json` en producción vía bloque `location` en Nginx (`deny all;` o restricción por IP); protección de identidad de la documentación completa se refuerza en Lab 4 |
...
- Queda pendiente: revisión de `access.log` para R7, y captura antes/después del hardening de Nginx (Fase E) para R1 y R2.
